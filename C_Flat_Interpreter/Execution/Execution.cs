        ...
        foreach (var token in Tokens)
        {
            switch (token.Type)
            {
                case TokenType.Num:
                    ...
                    _logger.Information("Pushed {output} to output stack", _outStack.Peek());
                    break;
                case TokenType.Add:
                case TokenType.Sub:
                    ...
                    _logger.Information("Pushed {operator} to operator stack", _opStack.Peek());
                    break;
    ...
    private void Evaluate()
    {
        ...
        switch (_opType) //perform evaluation based on operator
        {
            case TokenType.Add: //if plus add op1 and op2
                _logger.Information("Adding {op1} and {op2}", _op1, _op2);
                _outStack.Push(_op1 + _op2);
                break;
            case TokenType.Sub: //if minus subtract op1 from op2
                _logger.Information("Subtracting {op1} from {op2}", _op1, _op2);
                _outStack.Push(_op2 - _op1);
                break;
            case TokenType.Multi: //if star multiply op1 and op2
                _logger.Information("Multiplying {op1} and {op2}", _op1, _op2);
                _outStack.Push(_op1 * _op2);
                break;
            case TokenType.Divide: //if divide, divide op2 by op1
                _logger.Information("Dividing {op2} by {op1}", _op2, _op1);
                _outStack.Push(_op2 / _op1);
                break;
            default:
                _logger.Error("Unknown Operator, throwing exception");
                throw new ArgumentException($"{_opType.ToString()} is not a valid operator to evaluate");
        }
        _logger.Information("Pushed result {result} to output stack \n", _outStack.Peek());
    }
}